...
void GameLevel::loadResources()
{
    ...
    auto gameCtrl = std::shared_ptr<GameController>(new GameController(CURRENT_LEVEL, this));
    gameCtrl->setName("GameController");
    SDMaddSceneObject(gameCtrl, true, false);

    auto gameDialog = std::shared_ptr<is::GameDialog>(new is::GameDialog(m_texDialog, m_fontSystem, this));
    ...
    auto player = std::shared_ptr<Player>(new Player(gameCtrl->m_timeUp, gameCtrl->m_restartTime, this));
    ...
    auto gameHud = std::shared_ptr<HUD>(new HUD(this));
    gameHud->setName("HUD");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
        if (mapIndex[dataIndex] == 0) // block mask
            SDMaddSceneObject(std::shared_ptr<Block>(new Block(0, Block::BlockType::BLOCK_NORMAL, 32.f * j, 32.f * i)), false, false, "Block");
        else if (mapIndex[dataIndex] == 1) // block transparent (crossing block)
            SDMaddSceneObject(std::shared_ptr<Block>(new Block(0, Block::BlockType::BLOCK_TRANSPARENT, 32.f * j, 32.f * i)), false, false, "Block");
        else if (mapIndex[dataIndex] == 2) // block horizontal moving
        {
            auto block = std::shared_ptr<Block>(new Block(&m_texTile, Block::BlockType::BLOCK_MOVE_HORIZ, 32.f * j, 32.f * i));
            block->setMoveHorizontal(true);
            SDMaddSceneObject(block, false, true, "Block"); // Draw but don't update it. the GameController will update it
        }
        else if (mapIndex[dataIndex] == 3) // block vertical moving
        {
            auto block = std::shared_ptr<Block>(new Block(&m_texTile, Block::BlockType::BLOCK_MOVE_VERTI, 32.f * j, 32.f * i));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // Create background
    // Here the size of the scene is expressed in boxes because of the Tile Map.
    // 1 box = 32 pixels then we do a conversion so that the background stretches in
    // relation to the size of the scene in pixels.
    m_sceneWidth *= 32;
    m_sceneHeight *= 32;
    auto bg = std::shared_ptr<is::Background>(new is::Background(m_texBackground, 0.f, 0.f, this));
    bg->setDepth(is::DepthObject::VERY_BIG_DEPTH);
    SDMaddSceneObject(bg); // We add the object in the SDM container

    // the size is converted back to a box for the rest of the operations.
    m_sceneWidth /= 32;
    m_sceneHeight /= 32;
    ...
    while (!stop)
    {
        auto tile = std::shared_ptr<LevelTile>(new LevelTile(m_texTile, this));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    auto lvlTitle = std::shared_ptr<LevelTitle>(new LevelTitle(m_fontLevel, m_viewX, m_viewY,
                                                    ...
                                                    is::numToStr(CURRENT_LEVEL + 1), this));
    lvlTitle->setDepth(-5);
