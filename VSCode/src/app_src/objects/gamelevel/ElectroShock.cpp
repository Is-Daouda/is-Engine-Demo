...
ElectroShock::ElectroShock(float x, float y, is::GameDisplay *scene):
    MainObject(x, y),
    m_scene(scene)
{
    m_imageScale = 0.f;
    m_circleElectro.setRadius(32.f);
    m_circleElectro.setOutlineThickness(10.f);
...
void ElectroShock::step(float const &DELTA_TIME)
{
    if (static_cast<GameController*>(m_scene->SDMgetObject("GameController"))->canActivate())
    {
        ...
        is::setSFMLObjFillColor(m_circleElectro, sf::Color(255, 255, 255, m_imageAlpha));
        is::setSFMLObjOutlineColor(m_circleElectro, sf::Color(255, 255, 0, m_imageAlpha));
    }
}
