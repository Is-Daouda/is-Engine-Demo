#include "app_src/objects/gamelevel/GameController.h"
        ...
        if (is::instanceExist(*it))
        {
            if (m_scene->inViewRec(it->get(), false))
            {
                ...
                if ((*it)->getName() == "Block")
                {
                        ...
                        if (player->placeMetting(0, -yPrecision, currentBlock))
                        {
                            if (player->getVspAcc() < 0.f)
                            {
                                ...
                                else // blocks that give Bonuses
                                {
                                    ...
                                    {
                                        float blockX(currentBlock->getX()), blockY(currentBlock->getY());
                                        ...
                                        switch(currentBlock->getType())
                                        {
                                            case Block::BLOCK_COIN:
                                                sndName = "coin";
                                                addBonus();
                                                m_scene->SDMaddSceneObject(std::make_shared<Effect>(Effect::COIN_FLY, blockX + 4.f, blockY - 16.f, m_scene));
                                            break;
                                            ...
                                            case Block::BLOCK_MUSHROOM:
                                                ...
                                                {
                                                    m_scene->SDMaddSceneObject(std::make_shared<Bonus>(Bonus::MUSHROOM, blockX, blockY, m_scene));
                                                }
                                                else m_scene->SDMaddSceneObject(std::make_shared<Bonus>(Bonus::FIRE_FLOWER, blockX, blockY, m_scene));
                                            break;
                                            case Block::BLOCK_MUSHROOM_1UP:
                                                m_scene->SDMaddSceneObject(std::make_shared<Bonus>(Bonus::MUSHROOM_1UP, blockX, blockY, m_scene));
                                                currentBlock->setVisible(true);
                                            break;
                                            case Block::BLOCK_STARMAN:
                                                m_scene->SDMaddSceneObject(std::make_shared<Bonus>(Bonus::STARMAN, blockX, blockY, m_scene));
                                            break;
                                        }
                    ...
                    {
                        float blockX(currentBlock->getX()), blockY(currentBlock->getY() + 16.f);
                        m_scene->getGameSystem().m_currentScore += 50;

                        // create blocks debris
                        m_scene->SDMaddSceneObject(std::make_shared<Effect>(Effect::BLOCK_DEBRIS, blockX, blockY, m_scene, 4.f, 4.f));
                        m_scene->SDMaddSceneObject(std::make_shared<Effect>(Effect::BLOCK_DEBRIS, blockX, blockY, m_scene, 4.f, -4.f));
                        m_scene->SDMaddSceneObject(std::make_shared<Effect>(Effect::BLOCK_DEBRIS, blockX, blockY, m_scene, 1.5f, 1.5f));
                        m_scene->SDMaddSceneObject(std::make_shared<Effect>(Effect::BLOCK_DEBRIS, blockX, blockY, m_scene, 1.5f, -1.5f));
...
void GameController::pauseGame()
{
    ...
    if (((((m_scene->getGameSystem().keyIsPressed(is::GameConfig::KEY_VALIDATION_KEYBOARD) && !m_scene->getGameSystem().m_keyIsPressed) ||
