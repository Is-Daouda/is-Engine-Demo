...
Player::Player(bool &timeUp, int &restartTime, is::GameDisplay *scene):
    MainObject(),
    Visibility(),
    Health(4),
    m_scene(scene),
        ...
        if (m_isActive)
        {
            ...
            if (gameKey->m_keyBPressed && !gameKey->m_keyBUsed)
            {
                ...
                if (m_scene->SDMgetObject("ElectroShock") == nullptr)
                {
                    ...
                    auto electroShock = std::shared_ptr<ElectroShock>(new ElectroShock(getSpriteX(), getSpriteY(), m_scene));
